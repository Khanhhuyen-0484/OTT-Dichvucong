...
function normalizeDossier(dossier) {
  ...
  return {
    ...
    citizenName: String(dossier.citizenName || dossier.formData?.fullName || "Ngu?i d?n").trim(),
...
async function syncFromApplication(application) {
  ...
  const item = normalizeDossier({
    ...
    citizenName: application.formData?.fullName || application.citizenName || "Ngu?i d?n",
