...
exports.getPaymentStatus = async (req, res) => { const application = await findByCode(req.params.applicationCode); if (!application) return res.status(404).json({ message: "Không tìm thấy hồ sơ" }); res.json({ applicationCode: application.applicationCode, dossierId: application.dossierId, paymentStatus: application.paymentStatus || "UNPAID", status: application.status || "PENDING", paymentExpireAt: application.paymentExpireAt || null }); };
exports.mockPaymentComplete = async (req, res) => { const application = await findByCode(req.params.applicationCode); if (!application) return res.status(404).json({ message: "Không tìm thấy hồ sơ" }); const now = nowIso(); const timeline = pushTimeline(application, { status: application.status || "PENDING", action: "payment", note: "Thanh toán thành công", actor: userId(req) || "user", createdAt: now }); const next = await updateByCode(application.dossierId || req.params.applicationCode, { ...application, paymentStatus: "PAID", paymentCompletedAt: now, updatedAt: now, timeline, history: timeline }); res.json({ message: "Thanh toán thành công", paymentStatus: "PAID", application: next }); };
