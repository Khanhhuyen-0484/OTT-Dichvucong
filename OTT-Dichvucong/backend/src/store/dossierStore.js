...
function normalizeDossier(dossier) {
  ...
  const id = String(dossier.id || dossier.applicationCode || dossier.dossierCode || "").trim();
  if (!id) return null;
  return {
    ...dossier,
    id,
    applicationCode: dossier.applicationCode || id,
...
async function syncFromApplication(application) { const client = getClient(); const item = normalizeDossier({ id: application.applicationCode, applicationCode: application.applicationCode, serviceId: application.serviceId, serviceName: application.serviceName, citizenName: application.formData?.fullName || application.citizenName || "Người dân", phone: application.formData?.phone || application.phone || "", email: application.formData?.email || application.email || "", status: application.status || "new", paymentStatus: application.paymentStatus || "unpaid", timeline: application.history || [], createdAt: application.createdAt || new Date().toISOString(), updatedAt: application.updatedAt || new Date().toISOString(), source: "service_application" }); await client.send(new PutCommand({ TableName: TABLE_NAME, Item: item })); return item; }
