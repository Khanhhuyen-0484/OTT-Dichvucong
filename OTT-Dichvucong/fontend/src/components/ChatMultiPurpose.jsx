...
function ChatMultiPurpose({
  ...
  return (
    <div className="flex-1 min-w-0 p-3 sm:p-4 md:p-5 flex flex-col bg-[#F5F7FA] relative h-full">
      ...
      {!activeRoom ? (
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto rounded-xl border border-slate-200 bg-[#F5F7FA] p-3 sm:p-4 space-y-2">
            {messages.map((m, idx) => {
              ...
              return (
                  <div
                  key={m.id}
                  className={`group flex ${isMine ? "justify-end" : "justify-start"} items-end gap-2 px-1 sm:px-2`}
                ...
                >
                  ...
                  {/* Message Container */}
                  <div className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-[90vw] sm:max-w-[75%] lg:max-w-[450px] relative`}>
                      ...
                      <div className={`mb-1 inline-flex gap-1 rounded-full bg-white px-2 py-1 shadow-lg ring-1 ring-slate-200 ${
                        ...
                        {reactionOptions.map((emoji) => (
                          <button
                            ...
                            className="text-xs transition-transform hover:scale-125 active:scale-100 cursor-pointer disabled:opacity-50"
                            title="React"
                          >
                            {emoji}
                          </button>
                    ...
                    <div className="relative">
                      <Bubble
                        ...
                        location={m.unsentForAll ? null : m.location}
                        replyTo={m.replyTo}
          ...
          <form onSubmit={sendRoom} className="mt-3 flex flex-col gap-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-3">
            {/* Toolbar - Tính năng */}
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pb-2 border-b border-slate-200/50">
              {/* Nút Gửi Ảnh */}
              <label
                className="p-2.5 sm:p-3 h-11 w-11 rounded-full hover:bg-slate-100 cursor-pointer transition text-slate-600 hover:text-slate-800 flex items-center justify-center"
                title="Gửi ảnh"
              >
        ...
        <div className="fixed inset-0 z-[65] flex justify-end bg-black/30">
          <div className="h-full w-full max-w-md bg-white shadow-xl p-4 overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                {(myGroupRole === "owner" || myGroupRole === "deputy") && (
                  <button
                    type="button"
                    onClick={() => performGroupAction("leave")}
                    className="flex-1 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700"
                  >
                    Rời nhóm
                  </button>
                )}
                {myGroupRole === "owner" && (
                  <button
                    ...
                    className="flex-1 rounded-lg border border-red-200 bg-red-100 px-3 py-2 text-sm font-semibold text-red-700"
                  >
                    Giải tán nhóm
                  </button>
                )}
