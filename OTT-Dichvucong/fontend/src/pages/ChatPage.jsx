      ...
      <div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="max-h-72 space-y-2 overflow-y-auto pr-1 custom-scrollbar">
          {rooms.filter((r) => r.id !== activeRoomId).map((r) => (
            ...
            >
              {r.type === "group" ? `👥 ${r.name || "Nhóm"}` : `👤 ${r.members?.find((m) => m.id !== userId)?.fullName || "Người dùng"}`}
            </button>
          ))}
        </div>
      </div>
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    socket.on("new-message", (msg) => {
      ...
      else if (msg.from === "staff") setStaffUnread(prev => prev + 1);
    });
  ...
  }, [ready, user, tabState, loadRooms, loadStaff]);
  ...
  const startVideoCall = useCallback(() => {
    ...
    setVideoCallState({
      roomId: `call_${activeRoomId}_${Date.now()}`,
  ...
  const rejectCall = () => {
    if (incomingCall) {
      connectSocket().emit("call-rejected", { toUserId: incomingCall.callerUserId });
    }
    setIncomingCall(null);
  };
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-slate-50">
      ...
      <main className="flex min-h-0 flex-1 flex-col px-4 py-4 mx-auto w-full max-w-7xl">
        ...
        <div className="mb-4 flex gap-2 rounded-2xl bg-slate-200/50 p-1.5 border border-slate-200">
          <button onClick={() => setTabState("multi")} className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold transition-all ${tabState === "multi" ? "bg-white text-[#003366] shadow-sm" : "text-slate-500 hover:bg-white/50"}`}>
             Phòng Chat & Nhóm
          </button>
          <button onClick={() => { setTabState("staff"); setStaffUnread(0); }} className={`flex-1 relative flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold transition-all ${tabState === "staff" ? "bg-white text-[#003366] shadow-sm" : "text-slate-500 hover:bg-white/50"}`}>
             Hỗ trợ Cán bộ
            {staffUnread > 0 && <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[10px] text-white animate-bounce shadow-lg">!</span>}
          </button>
        </div>
          ...
          {tabState === "multi" ? (
            <div className="grid h-full gap-4 lg:grid-cols-12">
              <div className="lg:col-span-4 h-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <ContactList 
                  ...
                  openDirectChat={async (id) => { const {data} = await ensureDirectRoom(id); setActiveRoomId(data.room.id); loadRooms(); }}
                  setShowGroupModal={() => setShowGroupModal(true)} user={user}
                />
              </div>

              <div className="lg:col-span-8 h-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <ChatMultiPurpose 
                  activeRoom={activeRoom} user={user} roomInput={roomInput} setRoomInput={setRoomInput}
                  sendRoom={sendRoom} onStartVideoCall={startVideoCall} chatEndRef={chatEndRef}
                  ...
                  doMessageAction={async (action, mid) => {
                    ...
                    loadRooms(); setMessageMenuId(null);
                  }}
                  ...
                  onReplyMessage={setReplyToMessage} replyToMessage={replyToMessage} clearReply={() => setReplyToMessage(null)}
                />
              </div>
            </div>
          ) : (
            <div className="h-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm flex flex-col">
              <div className="bg-[#003366] p-5 text-white flex justify-between items-center shadow-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-blue-200 flex items-center gap-1"><span className="h-1.5 w-1.5 bg-green-400 rounded-full animate-pulse"/> Sẵn sàng hỗ trợ</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={async (e) => { 
                ...
                <input value={staffInput} onChange={e => setStaffInput(e.target.value)} placeholder="Nhập thắc mắc về thủ tục hành chính..." className="flex-1 rounded-2xl bg-slate-100 border-none px-5 py-3 text-sm focus:ring-2 focus:ring-[#003366] transition-all" />
                <button type="submit" disabled={staffLoading} className="rounded-2xl bg-[#003366] px-6 text-white font-bold transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2">
      ...
      {forwardingMessageId && (
        <ForwardModal 
          ...
          doForward={async (tid) => { await forwardRoomMessage(activeRoomId, forwardingMessageId, tid); setForwardingMessageId(null); loadRooms(); }} 
        />
      )}
