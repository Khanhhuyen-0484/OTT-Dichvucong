...
import { ArrowRight, BadgeCheck, CircleDollarSign, Clock3, FileText, LayoutGrid, Search, SlidersHorizontal, Sparkles, X } from "lucide-react";
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        <section style={styles.hero}>
          <div style={styles.heroHeader}>
            <div>
              ...
              <div style={{ marginTop: 12 }}><BackToDashboardButton variant="soft" /></div>
              <h1 style={styles.title}>Tìm dịch vụ nhanh hơn, rõ ràng hơn</h1>
              <p style={styles.desc}>Giao diện giúp người dân tra cứu dịch vụ theo từ khóa, nhóm thủ tục và mức phí trên mọi thiết bị.</p>
            </div>
          ...
          <div style={styles.toolbar}>
            <div style={styles.searchBox}>
              ...
              {query ? <button type="button" onClick={() => setQuery("")} style={styles.clearBtn} aria-label="Xóa tìm kiếm"><X size={16} /></button> : null}
            </div>
        ...
        {loading ? (
          <div style={styles.grid}>{Array.from({ length: 6 }).map((_, index) => <div key={index} style={styles.skeletonCard} />)}</div>
        ) : services.length === 0 ? (
              ...
              return (
                <Link key={id} to={`/services/${id}`} style={styles.card}>
                  ...
                  <div style={styles.metaRow}>
                    ...
                    <span style={styles.metaItem}><BadgeCheck size={14} />Xem chi tiết thủ tục</span>
                  </div>
                  <div style={styles.actionRow}>
                    <span style={styles.actionText}>Mở dịch vụ để xem hồ sơ, quy trình và nộp online</span>
                    <span style={styles.actionBtn}>Mở dịch vụ <ArrowRight size={14} /></span>
                  </div>
                </Link>
              );
            })}
...
const styles = {
  page: { minHeight: "100vh", background: "linear-gradient(180deg, #f8fafc 0%, #eef4fb 100%)" },
  container: { maxWidth: 1200, margin: "0 auto", padding: "32px 16px 48px" },
  hero: { background: "#fff", border: "1px solid #e2e8f0", borderRadius: 24, padding: 24, boxShadow: "0 10px 30px rgba(15, 23, 42, 0.05)", marginBottom: 20 },
  heroHeader: { display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: 16, alignItems: "center", marginBottom: 18 },
  heroBadge: { display: "inline-flex", alignItems: "center", borderRadius: 999, padding: "6px 12px", background: "#eff6ff", color: "#1d4ed8", fontSize: 12, fontWeight: 800, marginBottom: 12 },
  ...
  heroHighlight: { display: "flex", alignItems: "flex-start", gap: 10, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 20, padding: 16, color: "#334155", fontWeight: 600, lineHeight: 1.6 },
  toolbar: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12, marginBottom: 18 },
  searchBox: { display: "flex", alignItems: "center", gap: 10, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 16, padding: "0 14px", minHeight: 56 },
  filterBox: { display: "flex", alignItems: "center", gap: 10, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 16, padding: "0 14px", minHeight: 56 },
  searchInput: { width: "100%", height: 52, border: "none", outline: "none", background: "transparent", fontSize: 14 },
  clearBtn: { width: 30, height: 30, borderRadius: 999, border: "none", background: "#e2e8f0", color: "#475569", display: "inline-flex", alignItems: "center", justifyContent: "center", cursor: "pointer" },
  select: { width: "100%", height: 52, border: "none", outline: "none", background: "transparent", fontSize: 14, color: "#0f172a" },
  statsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 },
  statCard: { display: "flex", alignItems: "center", gap: 12, padding: 16, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 18 },
  statIcon: { width: 42, height: 42, borderRadius: 14, background: "#eff6ff", color: "#1d4ed8", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  card: { background: "#fff", borderRadius: 20, padding: 20, border: "1px solid #e2e8f0", textDecoration: "none", color: "#0f172a", boxShadow: "0 8px 24px rgba(15, 23, 42, 0.04)" },
  ...
  pricePill: { display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 999, background: "#f8fafc", border: "1px solid #e2e8f0", color: "#475569", fontSize: 12, fontWeight: 700 },
  ...
  metaItem: { display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 10px", borderRadius: 999, background: "#f8fafc", color: "#64748b", fontSize: 12, fontWeight: 600 },
  ...
  emptyState: { background: "#fff", border: "1px dashed #cbd5e1", borderRadius: 24, padding: 44, textAlign: "center", color: "#0f172a", boxShadow: "0 8px 24px rgba(15, 23, 42, 0.04)" },
  emptyTitle: { margin: "14px 0 8px", fontSize: 20 },
  emptyDesc: { margin: 0, color: "#64748b", lineHeight: 1.7 },
  errorState: { background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1d4ed8", padding: 14, borderRadius: 16, marginBottom: 16, fontWeight: 700 },
  skeletonCard: { height: 220, background: "#fff", borderRadius: 22, border: "1px solid #e2e8f0" },
};
