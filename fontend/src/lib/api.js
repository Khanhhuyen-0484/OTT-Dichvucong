...
export async function togglePinRoomMessage(roomId, messageId) {
  try {
    ...
    if (status === 404) {
      return await api.post(`/chat/rooms/${roomId}/messages/${messageId}/unpin`);
    }
    throw err;
  }
}
...
export async function leaveGroup(roomId, memberId) {
  return await removeGroupMember(roomId, memberId);
}
