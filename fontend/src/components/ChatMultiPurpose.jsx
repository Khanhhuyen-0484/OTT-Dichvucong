...
function ChatMultiPurpose({
  ...
  const handleSendLocation = async () => {
    ...
    try {
      ...
      setLocationError(error?.message || "Không thể gửi vị trí.");
    } finally {
    ...
    <div className="relative flex h-full min-h-0 flex-col overflow-hidden bg-white">
      ...
      <form onSubmit={sendRoom} className="shrink-0 border-t border-slate-100 bg-white p-3 shadow-[0_-4px_12px_rgba(15,23,42,0.06)]">
        <div className="mb-2 flex items-center gap-3 border-b border-slate-100 pb-2 text-slate-500">
          ...
          <button type="button" className="hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-50" onClick={handleSendLocation} disabled={locationLoading} title={locationLoading ? "Đang lấy vị trí..." : "Gửi vị trí hiện tại"}><MapPinned className="h-5 w-5" /></button>
        ...
        {locationLoading ? (
          <div className="mb-2 rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 ring-1 ring-blue-100">
            Đang lấy vị trí hiện tại...
          </div>
        ) : locationError ? (
          <div className="mb-2 rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 ring-1 ring-red-100">
            {locationError}
          </div>
        ) : null}
        <div className="flex items-center gap-2">
